...
func (d *DotRenderer) writeFormat(format string, args ...interface{}) {
...
func (d *DotRenderer) RenderNode(name string, attrs map[string]string) {
	d.writeFormat("%s [label=\"%s\"%s]", name, name, d.formatAttrs(attrs))
}

func (d *DotRenderer) RenderEdge(from, to string, attrs map[string]string) {
	d.writeFormat("%s -> %s%s", from, to, d.formatAttrs(attrs))
}

func (d *DotRenderer) BeginSubgraph(name, label string) {
	d.writeFormat("subgraph %s {", name)
	d.indent += "\t"
	d.writeFormat("label=\"%s\"", label)
}
